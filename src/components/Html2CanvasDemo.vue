...
const formData = ref({
  name: '李四',
  title: 'Html2Canvas 演示',
  content: '这张海报是使用 html2canvas 生成的。它是一个流行的库，用于将网页截图。虽然功能强大，但在处理某些现代 CSS 效果时可能不如 Snapdom 完美。',
  theme: 'ocean'
});
...
const themes = {
  ocean: 'background: linear-gradient(135deg, #24c6dc 0%, #514a9d 100%);',
  fire: 'background: linear-gradient(135deg, #f12711 0%, #f5af19 100%);',
  forest: 'background: linear-gradient(135deg, #134e5e 0%, #71b280 100%);',
  night: 'background: linear-gradient(135deg, #0f2027 0%, #203a43 50%, #2c5364 100%);',
};
</script>
  ...
  <div class="demo-container">
    <div class="controls">
      ...
      <div class="form-group">
        ...
        <select v-model="formData.theme">
          ...
          <option value="fire">烈焰燃情</option>
          <option value="forest">迷雾森林</option>
          <option value="night">极地暗夜</option>
        </select>
      </div>
    ...
    <div class="result-area" v-if="generatedImage">
      ...
      <div class="result-image-wrapper">
        ...
        <a :href="generatedImage" download="html2canvas-poster.png" class="download-link">下载海报</a>
      </div>
    </div>
  </div>
</template>
